...
def string_list_please() -> list:
    ...
    list_string = ["i", "have", "skipped", "lunch", "for", "this"]
    return list_string


def dictionary_please() -> dict:
    """Returns a dictionary, anything you like."""
    light_dict = {"tritium": "3H", "radioluminescence": "isotope"}
    return light_dict
...
def set_it_on_fire(input_string="very naughty boy") -> str:
    ...
    """
    ablaze = list(input_string.upper())
    ablaze = "🔥".join(ablaze)
    return f"🔥{ablaze}🔥"
...
def make_filler_text_dictionary() -> Dict:
    ...
    for i in range(3, 8):
        ...
        for x in range(4):
            wordlink = url + str(i)
            addlinknumber = requests.get(wordlink)
            wd[i].append(addlinknumber.text)
